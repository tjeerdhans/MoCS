...
namespace MoCS.BuildService
{
    public class SubmitWatcher
    {
        ...
        private MoCS.Client.Business.Entities.IFileSystem _fileSystem;
        ...
        private void SaveStatusToDatabase(SubmitToProcess submit, SubmitResult result)
        {
            ...
            switch (result.Status)
            {
                case SubmitStatusCode.Success:  //1
                    _dataAccess.InsertSubmitStatus(submit.TeamID, submit.SubmitID, 1, null);
                ...
                case SubmitStatusCode.CompilationError: //20
                    _dataAccess.InsertSubmitStatus(submit.TeamID, submit.SubmitID, 20, details);
                    break;
                case SubmitStatusCode.ValidationError:  //21
                    _dataAccess.InsertSubmitStatus(submit.TeamID, submit.SubmitID, 21, details);
                    break;
                case SubmitStatusCode.TestError:        //22
                    _dataAccess.InsertSubmitStatus(submit.TeamID, submit.SubmitID, 22, details);
                    break;
                case SubmitStatusCode.ServerError:      //23
                    _dataAccess.InsertSubmitStatus(submit.TeamID, submit.SubmitID, 23, details);
                    break;
                case SubmitStatusCode.Unknown:          //99
                    _dataAccess.InsertSubmitStatus(submit.TeamID, submit.SubmitID, 90, details);
                    break;
                default:
                    _dataAccess.InsertSubmitStatus(submit.TeamID, submit.SubmitID, 99, details);
                    break;
            }
            _dataAccess.SetTeamSubmitToFinished(submit.SubmitID);
        ...
        {
            List<SubmitToProcess> submits = _dataAccess.GetUnprocessedSubmits();
